...
describe('Covid-19 Estimator', () => {
  ...
  it('Should pass challenge 1', () => {
    const result = covid19ImpactEstimator(testData.data);
    expect(result.data).toEqual(expect.objectContaining(testData.data));
    expect(testData.estimate.impact).toEqual(expect.objectContaining(result.impact));
    expect(testData.estimate.severeImpact).toEqual(expect.objectContaining(result.severeImpact));
  });
});
